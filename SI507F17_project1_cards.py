...
class Deck(object):
    ...
    def replace_card(self, card):
        ...
        for c in self.cards:
            ...
            # appends the string that represents
            # that card to the empty list
        if card.__str__() not in card_strs:
            # if the string representing this card is not in
            # the list already
...
# Because it's written in a silly way, there are a bunch of
# edge cases of sorts.
def play_war_game(testing=False):
    # Call this with testing = True and it won't print
    # out all the game mechanics, which makes it easier to see tests.
    player1 = Deck()
    player2 = Deck()
